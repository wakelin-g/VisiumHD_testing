...
def normalize_counts(
    ...
    regressed = np.asarray(
        NaiveDE.regress_out(sizefactors, stabilized, sizefactorcol).T
    )
    if layer is None:
        adata.X = regressed
    else:
...
def bh_adjust(pvals):
    ...
    alpha = np.minimum(
        1,
        np.maximum.accumulate(len(pvals) / np.arange(1, len(pvals) + 1) * pvals[order]),
    )

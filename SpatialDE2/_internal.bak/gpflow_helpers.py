        ...
        for v in np.linspace(minvar, np.minimum(1, 0.9 * maxvar), ncomponents):
            k = Spectral(
                ...
                lengthscales=gpflow.Parameter(
                    varinit,
                    transform=tfp.bijectors.Sigmoid(
                        low=0.1 * min_1nndist, high=datarange
                    ),
                ),
                periods=gpflow.Parameter(
                    periodinit,
                    transform=tfp.bijectors.Sigmoid(low=minperiod, high=2 * datarange),
                ),
            )
            kernels.append(k)
...
class GeneGP(Model):
    def __init__(self, model: GeneGPModel, minimize_fun, *args, **kwargs):
        ...
        self.__invHess = None
        self.model.likelihood.variance.assign(
            tf.math.reduce_variance(self.model.data[1])
        )
    ...
    def plot_power_spectrum(self, xlim: float = None, ylim: float = None, **kwargs):
        return self.model.kernel.spectral_mixture.plot_power_spectrum(
            xlim, ylim, **kwargs
        )

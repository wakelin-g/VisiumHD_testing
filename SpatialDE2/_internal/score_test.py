        ...
        if self._yidx is not None:
            ...
            if tf.size(tf.shape(self._K)) > 2:
                ...
                idx = tf.concat(
                    (
                        tf.repeat(
                            tf.range(bdim, dtype=self._yidx.dtype),
                            tf.square(tf.size(self._yidx)),
                        )[:, tf.newaxis],
                        idx,
                    ),
                    axis=1,
...
class NegativeBinomialScoreTest(ScoreTest):
        ...
        res = minimize(
            ...
            x0=[
                ...
                tf.math.log(
                    tf.maximum(
                        1e-8,
                        self._moments_dispersion_estimate(scaledy, self.sizefactors),
                    )
                ),
            ],
    ...
    def _do_test(
        ...
        stat = 0.5 * tf.reduce_sum(
            (rawy / mu - 1) * W * tf.tensordot(K, W * (rawy / mu - 1), axes=(-1, -1)),
            axis=-1,
        )

        P = tf.linalg.diag(W) - W[:, tf.newaxis] * W[tf.newaxis, :] / tf.reduce_sum(W)
        PK = W[:, tf.newaxis] * K - W[:, tf.newaxis] * (
            (W[tf.newaxis, :] @ K) / tf.reduce_sum(W)
        )

...
class Spectral(Stationary):
    def __init__(self, variance=1.0, lengthscales=1, periods=1, **kwargs):
        super().__init__(
            ...
            lengthscales=Parameter(
                lengthscales, transform=positive(lower=to_default_float(1e-6))
            ),
            **kwargs,
        )
        self.periods = Parameter(
            periods, transform=positive(lower=to_default_float(1e-6))
        )
    ...
    def log_power_spectrum(self, s):
        ...
        scale_diag = tf.broadcast_to(
            self.lengthscales / (0.25 * np.pi**2), (s.shape[1],)
        )
        mvd = tfp.distributions.MultivariateNormalDiag(
            loc=1 / loc, scale_diag=1 / scale_diag
        )
        return tf.math.log(
            tf.constant(0.5, dtype=self.variance.dtype)
        ) + tf.reduce_logsumexp([mvd.log_prob(s), mvd.log_prob(-s)], axis=0)
...
class SpectralMixture(Sum):
    ...
    def plot_power_spectrum(self, xlim: float = None, ylim: float = None, **kwargs):
        if xlim is None or ylim is None:
            ...
            maxfreq = tf.stack(
                [maxfreq, tf.range(maxfreq.shape[0], dtype=tf.int32)], axis=1
            )
        ...
        dim = max(
            [k.lengthscales.ndim for k in self.kernels]
            + [k.periods.ndim for k in self.kernels]
        )
        fig, ax = plt.subplots()
        if dim < 1:
            ...
            x, y = tf.meshgrid(
                tf.linspace(0.0, xlim, 1000), tf.linspace(0.0, ylim, 1000)
            )
            ps = tf.reshape(
            ...
            pos = ax.contourf(
                ...
                y,
                ps,
                levels=tf.linspace(tf.reduce_min(ps), tf.reduce_max(ps), 100),
                **kwargs,
            )

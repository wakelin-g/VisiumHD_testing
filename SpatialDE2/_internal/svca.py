...
class SVCA(tf.Module):
    ...
    _fracvar_interact = namedtuple(
        "FractionVraiance", "intrinsic environmental interaction noise"
    )
    ...
    def __init__(
        ...
        self._current_expression = tf.Variable(
            tf.zeros((self._ncells, self._ngenes - 1), dtype=default_float()),
            trainable=False,
        )
        self.intrinsic_variance_matrix = tf.Variable(
            tf.zeros((self._ncells, self._ncells), dtype=default_float()),
            trainable=False,
        )
        self._sigmas = gpflow.Parameter(
            tf.ones((4,), dtype=default_float()),
            transform=gpflow.utilities.positive(lower=1e-9),
        )
        self._currentgene = tf.Variable(0, dtype=tf.int32, trainable=False)
        self.muhat = tf.Variable(
            tf.ones((self._ncells,), dtype=default_float()), trainable=False
        )

        self.kernel = kernel

        self._opt = MultiScipyOptimizer(
            lambda: -self.profile_log_reml(), self.trainable_variables
        )
    ...
    def currentgene(self, gene: int):
        ...
        intvar = tf.matmul(
            self._current_expression, self._current_expression, transpose_b=True
        )
        ...
        ldet2 = tf.math.log(
            tf.reduce_sum(
                tf.linalg.cholesky_solve(
                    Vchol, tf.ones((self._ncells, 1), dtype=default_float())
                )
            )
        )
    ...
    def _alphahat(self, Vchol):
        ...
        VinvX = tf.linalg.cholesky_solve(
            Vchol, tf.ones((self._ncells, 1), dtype=default_float())
        )
    ...
    def _betahat(self, Vchol):
        return tf.squeeze(
            self.D() @ tf.linalg.cholesky_solve(Vchol, self._r(Vchol)[:, tf.newaxis])
        )
    ...
    def interaction_variance_matrix(self):
        ...
        intmat = envmat @ tf.matmul(
            self.intrinsic_variance_matrix, envmat, transpose_b=True
        )
...
class SVCAInteractionScoreTest(ScoreTest):
    ...
    def _do_test(residual, V, dV, interaction_mat):
        ...
        stat = 0.5 * tf.tensordot(
            Vinvres,
            tf.tensordot(interaction_mat, Vinvres, axes=(-1, -1)),
            axes=(-1, -1),
        )
        ...
        I_theta_theta = tf.reduce_sum(
            tf.linalg.matrix_transpose(P_dV[tf.newaxis, ...])
            * P_dV[:, tf.newaxis, ...],
            axis=[-2, -1],
        )
        ...
        I_tau_tau_tilde = 0.5 * (
            ...
            - tf.tensordot(
                I_tau_theta,
                tf.squeeze(
                    tf.linalg.solve(I_theta_theta, I_tau_theta[..., tf.newaxis])
                ),
                axes=(-1, -1),
            )
        )

...
def test_spatial_interactions(
    ...
    test = SVCAInteractionScoreTest(
        dense_slice(adata.X if layer is None else adata.layers[layer]),
        X,
        sizefactors,
        kernel,
    )
    ...
    results.loc[results.pval > 1, "pval"] = (
...
def fit_spatial_interactions(
    ...
    if (
        ...
        warnings.warn(
            "SVCA parameters not found in adata. Performing ab initio fitting."
        )
        if genes is None:
    ...
    model = SVCA(
        dense_slice(
            adata.X
            if adata.uns["svca_layer"] is None
            else adata.layers[adata.uns["svca_layer"]]
        ),
        X,
        sizefactors,
        kernel,

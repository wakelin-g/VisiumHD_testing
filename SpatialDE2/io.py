...
def read_spaceranger(spaceranger_out_dir: str, read_images: bool = True) -> AnnData:
    ...
    """
    fname = glob.glob(
        os.path.join(spaceranger_out_dir, "*filtered_feature_bc_matrix.h5")
    )
    if len(fname) == 0:
    ...
    adata.obs = tissue_positions.drop(
        ["pxl_row_in_fullres", "pxl_col_in_fullres"], axis=1
    )

    with open(
        os.path.join(spaceranger_out_dir, "spatial", "scalefactors_json.json"), "r"
    ) as f:
        meta = json.load(f)
